...
```{r, context = "server", eval=params$export, fig.height=11, out.height=11}
are_out_age <- areOutAge(full_records)
data_to_use <- dataToUse(full_records, params$quality_completed)
fun_y <- summaryFun(params$quality_type)
completed <- params$quality_completed
...
```{r}
n_missing <- purrr::map_int(full_records, ~sum(is.na(.)))
...
data_miss <- full_records %>%
  ...
  tidyr::pivot_longer(-center, names_to = "field")
...
data_miss <- data_miss %>%
  dplyr::left_join(n_each) %>%
  dplyr::mutate(prop = round(value / n, 2))
```
