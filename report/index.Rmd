...
data_path <- dir_create(here("data"))
file_path <- file.path(data_path, "tipnet_full.rds")

tipnet_full <- read_redcap(
  url = tipnet_redcap_url()
)

if (tipnet_full$data$success && tipnet_full$meta_data$success) {
  write_rds(tipnet_full, path = file_path)
} else {
  ui_warn("Connection error to REDCap TIPNet database.")
}
```
...
tipnet_full <- read_rds(file_path)

tipnet <- tidy_data_from(tipnet_full)
