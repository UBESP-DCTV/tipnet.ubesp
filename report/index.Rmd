...
# if (params$dev) {
# # this cannot work into the setup chunk because it is
# # the chunk which load params  variable...
...
path_to_data <- tipnet.ubesp:::data_path()
last_file <- file.path(path_to_data, "tipnet.rds")
```
...
tipnet_meta <- file.path(path_to_data, "tipnet_meta.rds") %>% 
  readr::read_rds()
...
output$report <- downloadHandler("tipnet-repor.html",
  ...
  content = function(file) {
    ...
    out <- readr::read_lines(temp_report) %>% 
      stringr::str_replace("^runtime: shiny_prerendered$", "") %>% 
      stringr::str_replace_all(
        "\\.tabset \\.tabset-fade \\.tabset-pills",
        ""
      ) 
    readr::write_lines(out, temp_report)
