...
stopifnot(here::here() == normalizePath(".."))
...
last_update <- list.files(tipnet.ubesp:::data_path(),
  pattern = "^\\d{4}-\\d{2}-\\d{2}-tipnet.*\\.rds$",
  full.names = TRUE
) %>% 
  purrr::map_df(fs::file_info, follow = TRUE) %>% 
  dplyr::pull(.data[["modification_time"]]) %>% 
  max() %>% 
  as.Date()

expired <- (lubridate::today() - last_update) > params$max_days_update

if (
  (
    (expired && params$auto_update) || params$force_update
  ) &&
...
```{r data, include=FALSE, cache=params$cache, cache.extra=tools::md5sum(last_update)}
...
data_name <- paste0(last_update, "-", "tipnet.rds") 
tipnet <- readr::read_rds(
  file.path(tipnet.ubesp:::data_path(), data_name)
)
...
aux_out <- full_records %>% 
  dplyr::select(
    center, codpat, redcap_repeat_instance,
    where(is.numeric)
  ) %>% 
  dplyr::mutate_at(
    dplyr::vars(-center, -codpat, -redcap_repeat_instance),
  ...
  tidyr::pivot_longer(-c(center, codpat, redcap_repeat_instance)) %>% 
