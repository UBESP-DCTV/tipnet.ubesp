...
```{r data, include=FALSE, cache=TRUE, cache.extra=tools::md5sum(tipnet_path)}
...
anagrafica <- get_sheet(tipnet, "anagrafica") %>% 
  mutate(
    years = as.period(interval(
        start = nascita_dt,
        end   = today())
      )$year,
    age_upto_18 = years <= 18,
    age_upto_16 = factor(years <= 16,
      levels = c(TRUE, FALSE),
      labels = c("age <= 16", "16 < age <= 18")
    )
  )
...
full_records <- anagrafica %>% 
  full_join(accettazione,
    by = c("codpat", "center"),
    suffix = c(".anagrafica", ".accettazione")
  ) %>%
  full_join(pim, by = c("codpat", "center", "redcap_repeat_instance")) %>% 
  full_join(dimissione,
    by = c("codpat", "center", "redcap_repeat_instance"),
    suffix = c(".pim", ".dimissione")
  ) %>% 
