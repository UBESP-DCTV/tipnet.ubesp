...
full_records <- accettazione %>% 
  full_join(anagrafica,
    by = c("codpat", "center"),
    suffix = c(".accettazione", ".anagrafica")
  ) %>%
  
  full_join(ingresso, by = c("codpat", "center", "redcap_repeat_instance")) %>% 
  full_join(pim,
    by = c("codpat", "center", "redcap_repeat_instance"),
    suffix = c(".ingresso", ".pim")
  ) %>% 
  
  full_join(pelod, by = c("codpat", "center", "redcap_repeat_instance")) %>% 
  full_join(aristotle,
    by = c("codpat", "center", "redcap_repeat_instance"),
    suffix = c(".pelod", ".aristotle")
  ) %>% 
  
  full_join(degenza, by = c("codpat", "center", "redcap_repeat_instance")) %>% 
  full_join(ventilazione,
    by = c("codpat", "center", "redcap_repeat_instance"),
    suffix = c(".degenza", ".ventilazione")
  ) %>% 
  
  full_join(infezione, by = c("codpat", "center", "redcap_repeat_instance")) %>% 
  full_join(dimissione,
    by = c("codpat", "center", "redcap_repeat_instance"),
    suffix = c(".infezione", ".dimissione")
  ) %>% 
  
  mutate_at(vars(starts_with("complete")), ~{. == "complete"}) %>% 
  mutate(
    eta = as.integer(eta),
    age_class = factor(
      dplyr::case_when(
